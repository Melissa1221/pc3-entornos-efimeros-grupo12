...
class TrendsAnalyzer:
    ...
    def get_operation_trends(self, days: int = 30) -> Dict[str, Any]:
        ...
        recent_ops = [
            op for op in self.data.get("operations", [])
            if datetime.fromisoformat(op["timestamp"].replace('Z', '+00:00')) > cutoff_date
        ]
        
        deploy_times = [op["duration_seconds"] for op in recent_ops 
                       if op["operation"] == "deploy" and op["status"] == "success"]
        destroy_times = [op["duration_seconds"] for op in recent_ops 
    ...
    def get_drift_trends(self, days: int = 30) -> Dict[str, Any]:
        ...
        recent_checks = [
            check for check in self.data.get("drift_checks", [])
            if datetime.fromisoformat(check["timestamp"].replace('Z', '+00:00')) > cutoff_date
        ]
        
        drift_percentages = [check["drift_percent"] for check in recent_checks 
                           if check["status"] == "drift_detected"]
        zero_drift_count = len([check for check in recent_checks 
                               if check["drift_percent"] == 0])
        
        compliance_rate = (zero_drift_count / len(recent_checks) * 100) if recent_checks else 0
        ...
        return {
            ...
            "drift_stats": self._calculate_stats(drift_percentages) if drift_percentages else None,
    ...
    def _group_operations_by_day(self, operations: List[Dict], days: int) -> List[Dict]:
        ...
        for i in range(days):
            ...
            daily_data[date] = {"date": date, "deploys": 0, "destroys": 0, "failures": 0}
    ...
    def _group_drift_by_day(self, checks: List[Dict], days: int) -> List[Dict]:
        ...
        for day_data in daily_data.values():
            if day_data["total_checks"] > 0:
                day_data["compliance_rate"] = (day_data["zero_drift"] / day_data["total_checks"]) * 100
            else:
...
class DashboardGenerator:
    ...
    def generate_html_dashboard(self, output_file: str, days: int = 30) -> str:
        ...
        html_content = self._generate_html_template(operation_trends, drift_trends, days)
    ...
    def _generate_html_template(self, operation_trends: Dict, drift_trends: Dict, days: int) -> str:
...
def main():
    ...
    parser.add_argument('--metrics-file', default='metrics/operations.json',
                       help='Archivo de métricas JSON')
    parser.add_argument('--output', default='dashboard/trends.html',
