...
class CleanupMonitor:
    ...
    def _scan_containers(self) -> List[Dict]:
        ...
        try:
            cmd = ['docker', 'ps', '-a', '--filter', 'label=environment=ephemeral', 
                   '--format', '{{.Names}}\t{{.Status}}\t{{.CreatedAt}}\t{{.Labels}}']
    ...
    def _scan_volumes(self) -> List[Dict]:
        ...
        try:
            cmd = ['docker', 'volume', 'ls', '--filter', 'name=ephemeral-pr-', 
            ...
            for line in result.stdout.strip().split('\n'):
                ...
                if len(parts) >= 2:
                    volume_info = {
                        ...
                        'pr_number': self._extract_pr_number(parts[0]),
                        'age_hours': self._calculate_age_hours(parts[2]) if len(parts) > 2 else None
                    }
    ...
    def _scan_networks(self) -> List[Dict]:
        ...
        try:
            cmd = ['docker', 'network', 'ls', '--filter', 'name=ephemeral-pr-', 
                   '--format', '{{.Name}}\t{{.Driver}}\t{{.CreatedAt}}']
            result = subprocess.run(cmd, capture_output=True, text=True)
            ...
            for fmt in ['%Y-%m-%d %H:%M:%S %z', '%Y-%m-%d %H:%M:%S', '%Y-%m-%dT%H:%M:%S%z']:
                try:
                    ...
                    if created_time.tzinfo is None:
                        created_time = created_time.replace(tzinfo=self.current_time.tzinfo)
...
"""
        ...
        if analysis['cleanup_candidates']['containers']:
            ...
            for container in analysis['cleanup_candidates']['containers']:
                age = f"{container['age_hours']:.1f}" if container['age_hours'] else "N/A"
                pr = f"#{container['pr_number']}" if container['pr_number'] else "N/A"
                report += f"| {container['name']} | {container['status']} | {age} | {pr} |\n"
            report += "\n"
    ...
    def get_resource_summary(self) -> Dict[str, int]:
        ...
        return {
            ...
            'running_containers': len([c for c in resources['containers'] if 'Up' in c['status']]),
            'total_volumes': len(resources['volumes']),
            'total_networks': len(resources['networks']),
            'unique_prs': len(set(r['pr_number'] for r in 
                                resources['containers'] + resources['volumes'] + resources['networks']
                                if r['pr_number'] is not None))
        }


def main():
    parser = argparse.ArgumentParser(description='Monitor de limpieza de stacks efímeros')
    parser.add_argument('--max-age', type=int, default=72,
                       help='Edad máxima en horas para considerar limpieza')
    parser.add_argument('--report', action='store_true',
                       help='Generar reporte de limpieza')
    parser.add_argument('--summary', action='store_true',
                       help='Mostrar resumen de recursos')
    ...
    if args.summary:
        ...
        if args.json:
            ...
            print(f"Contenedores: {summary['total_containers']} (ejecutándose: {summary['running_containers']})")
        ...
        if args.json:
            ...
            if candidates['pr_numbers']:
                print(f"  PRs: {', '.join(f'#{pr}' for pr in sorted(candidates['pr_numbers']))}")
