...
class DockerManager:
    ...
    def get_containers_by_pr(pr_number: int) -> List[str]:
        ...
        try:
            cmd = [
                'docker', 'ps', '-a',
                '--filter', f'label=pr_number={pr_number}',
                '--format', '{{.Names}}'
            ]
    ...
    def get_volumes_by_pr(pr_number: int) -> List[str]:
        ...
        try:
            cmd = [
                'docker', 'volume', 'ls',
                '--filter', f'name=ephemeral-pr-{pr_number}',
                '--format', '{{.Name}}'
            ]
            result = subprocess.run(cmd, capture_output=True, text=True)
    ...
    def get_networks_by_pr(pr_number: int) -> List[str]:
        ...
        try:
            ...
            return [n for n in networks if n and n != 'bridge' and n != 'host' and n != 'none']
        except subprocess.CalledProcessError:
            return []
...
class TestCleanupOperations:
    ...
    def test_docker_manager_get_containers_with_results(self):
        ...
        with patch('subprocess.run') as mock_run:
            mock_run.return_value.stdout = "ephemeral-pr-123-app\nephemeral-pr-123-proxy\nephemeral-pr-123-db\n"
            mock_run.return_value.returncode = 0
            
            containers = DockerManager.get_containers_by_pr(123)
            expected = ['ephemeral-pr-123-app', 'ephemeral-pr-123-proxy', 'ephemeral-pr-123-db']
    ...
    def test_docker_manager_cleanup_orphaned_resources(self):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            with patch.object(DockerManager, 'get_containers_by_pr', return_value=['container1']):
                with patch.object(DockerManager, 'get_volumes_by_pr', return_value=['volume1']):
                    with patch.object(DockerManager, 'get_networks_by_pr', return_value=['network1']):
                        mock_run.return_value.returncode = 0
                        ...
                        assert mock_run.call_count == 3  # rm container, rm volume, rm network
    ...
    def test_terraform_validator_verify_state_not_empty(self):
        ...
        with patch('subprocess.run') as mock_run:
            mock_run.return_value.stdout = "docker_container.app\ndocker_container.proxy\n"
            mock_run.return_value.returncode = 0
...
class TestCleanupIntegration:
    ...
    def test_cleanup_with_orphaned_resources(self):
        ...
        with patch.object(DockerManager, 'get_containers_by_pr', return_value=['orphan-container']):
            with patch.object(DockerManager, 'get_volumes_by_pr', return_value=['orphan-volume']):
                with patch.object(DockerManager, 'get_networks_by_pr', return_value=['orphan-network']):
                    with patch('subprocess.run') as mock_run:
                        mock_run.return_value.returncode = 0
